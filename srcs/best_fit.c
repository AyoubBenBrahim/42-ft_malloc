...
{
  t_bin *new_free_bin = (t_bin *)((void *)best_fit_bin + BIN_HEADER_SIZE + size);
...
t_arena *find_best_fit_arena(t_bins_type binType, size_t size) {
  ...
  while (curr_arena) {
    if (curr_arena->bin_type == binType && curr_arena->free_size >= size &&
        !is_out_of_arena_boundary(curr_arena, size)) {
...
t_bin *get_last_bin_in_arena(t_arena *arena, t_bins_type binType) {
  return (binType == TINY) ? (t_bin *)((void *)arena->last_tiny_bin)
                           : (t_bin *)((void *)arena->last_small_bin);
}
...
t_bin *find_best_fit(t_bins_type binType, size_t size) {
  ...
  if (best_fit_bin) {
    ...
    best_fit_bin->magic_number = generateMagicNumber(best_fit_bin->parent_arena);
